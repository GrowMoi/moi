module TreeService
  class ContentsForTestFetcher
    ...
    def contents
      readings = @user.content_readings.map(&:content_id)
      learnings = @user.content_learnings.map(&:content_id)
      content_to_learn = readings - learnings
      Content.where( id: content_to_learn )
      # content_ids = [
